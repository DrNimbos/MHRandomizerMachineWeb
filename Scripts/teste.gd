...
var SELECT_BR = preload("res://cena/select_br.tscn")
...
func _ready():
	...
	dialog.start_typing_effect("Oi Hunter! Meu nome é Luna. Sou uma ex- Recepcionista da Guilda e pesquisadora.. Para ativar a maquina, aperte o sino na mesa!")
	trocarsprite("1")
...
func rolar():
	...
	if condi_hand.l_condi.is_empty():
		if esp_temp != "":
			label.text = MC_R + " " + esp_temp +" " + monstro_selecionado.nome + " com " + arma_selecionado
			dialog.start_typing_effect(monstro_selecionado.comentario2_br)
			...
			label.text = MC_R + " " + monstro_selecionado.nome + " com " + arma_selecionado
			dialog.start_typing_effect(monstro_selecionado.comentario2_br)
			trocarsprite(String(monstro_selecionado.reacao))
		else:
			label.text = MC_R + " " + temp +" " + monstro_selecionado.nome + " com " + arma_selecionado
			dialog.start_typing_effect(monstro_selecionado.comentario2_br)
			trocarsprite(String(monstro_selecionado.reacao))
		...
		if esp_temp != "":
			...
			label.text = MC_R + " " + monstro_selecionado.nome + " com " + arma_selecionado + " " + condicao
			dialog.start_typing_effect(monstro_selecionado.comentario2_br)
			trocarsprite(String(monstro_selecionado.reacao))
		else:
			label.text = MC_R + " " + temp +" " + monstro_selecionado.nome + " com " + arma_selecionado + " " + condicao
			dialog.start_typing_effect(monstro_selecionado.comentario2_br)
			trocarsprite(String(monstro_selecionado.reacao))
